...
Conv2D = functools.partial(  # pylint: disable=invalid-name
    tf.keras.layers.Conv2D, kernel_size=3, padding="same", use_bias=False
)
CondConv2D = functools.partial(  # pylint: disable=invalid-name
    ed.layers.CondConv2D, kernel_size=3, padding="same", use_bias=False
)
...
def _get_routing_weights(
    ...
    if use_softmax_top_k:
        ...
        # expert values.
        routing_weights = _rowwise_unsorted_segment_sum(
    ...
    gating_layer = tf.keras.layers.Dense(
        num_experts, activation=tf.nn.sigmoid if use_sigmoid_activation else None
    )
    ...
    if use_onehot_top_k:
        if k < 1:
            raise ValueError("Cannot perform top_k selection with k = {} < 1".format(k))
...
def basic_block(
    ...
    if cond_placement in ["all", "dropout"]:
        routing_weights = _get_routing_weights(
            y, num_experts, normalize_routing, routing_pooling, top_k, routing_fn
        )
        ...
        y = Conv2D(
            filters, strides=strides, kernel_regularizer=tf.keras.regularizers.l2(l2)
        )(y)
    ...
    if cond_placement == "all":
        ...
        y = Conv2D(filters, strides=1, kernel_regularizer=tf.keras.regularizers.l2(l2))(
            y
        )
    ...
    if not x.shape.is_compatible_with(y.shape):
        if cond_placement == "all":
            routing_weights = _get_routing_weights(
                x, num_experts, normalize_routing, routing_pooling, top_k, routing_fn
            )
            routing_weights_list.append(routing_weights)
            x = CondConv2D(
                filters,
...
def wide_resnet_condconv(
    ...
    if cond_placement == "all":
        routing_weights = _get_routing_weights(
            inputs, num_experts, normalize_routing, routing_pooling, top_k, routing_fn
        )
    ...
    if use_cond_dense:
        ...
        if reduce_dense_outputs:
            ...
            dense_output = ed.layers.CondDense(num_classes, num_experts=num_experts)(
                x, routing_weights
            )
        else:
