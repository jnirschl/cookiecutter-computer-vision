...
class WideResnetBatchensemble(tf.test.TestCase):
    def testWideResnetHyperBatchensemble(self):
        ...
        # build model
        # configs
        dict_ranges = {"min": 0.01, "max": 100.0}
        ...
        lambdas_config = networks.HyperBatchEnsembleLambdaConfig(
        ...
        e_models = networks.hyperbatchensemble_e_factory(
        ...
        model = networks.wide_resnet_hyperbatchensemble(
        ...
        input_lambdas = tf.random.uniform((batch_size,) + lambdas_config.input_shape)
