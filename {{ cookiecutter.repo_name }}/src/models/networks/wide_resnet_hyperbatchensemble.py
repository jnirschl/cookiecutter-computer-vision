...
class LambdaConfig:
    ...
    def __init__(
        self, ranges, key_to_index, dist=DEFAULT_DISTRIBUTION, encoding=DEFAULT_ENCODING
    ):
...
def e_factory(
    ...
    reg = tf.keras.regularizers.L2(l2=0.0)
    ...
    for i, e_head_dim in enumerate(e_head_dims):
        ...
        out = tf.keras.layers.Dense(
            ...
            kernel_regularizer=reg,
            bias_regularizer=reg,
            kernel_initializer=head_kernel_initializer,
        )(out)
...
def make_sign_initializer(random_sign_init):
    ...
    if random_sign_init > 0:
        ...
        return tf.keras.initializers.RandomNormal(mean=1.0, stddev=-random_sign_init)


def wide_resnet_hyperbatchensemble(
    input_shape,
    depth,
    width_multiplier,
    num_classes,
    ...
    def basic_block(inputs, filters, strides, version, block_name, l2_batchnorm_layer):
        ...
        assert (
            len(block_e_list) == 2 or len(block_e_list) == 3
        ...
        if version == 2:
            y = BatchNormalization(
                ...
                gamma_regularizer=tf.keras.regularizers.l2(l2_batchnorm_layer),
            )(y)
            y = tf.keras.layers.Activation("relu")(y)

        layer_index = 0
        layer_name = block_name + "_conv_{}/".format(layer_index)
        y = Conv2D(filters, strides=strides, name=layer_name)(
            [y, lambdas, block_e_list[layer_index]]
        )
        ...
        layer_index += 1
        layer_name = block_name + "_conv_{}/".format(layer_index)
        y = Conv2D(filters, strides=1, name=layer_name)(
            [y, lambdas, block_e_list[layer_index]]
        )
        ...
        if not x.shape.is_compatible_with(y.shape):
            layer_index += 1
            layer_name = block_name + "_conv_{}/".format(layer_index)
            x = Conv2D(filters, kernel_size=1, strides=strides, name=layer_name)(
                [x, lambdas, block_e_list[layer_index]]
            )
    ...
    def group(inputs, filters, strides, num_blocks, version, name, l2_batchnorm_layer):
        """Group of residual blocks."""

        # unpack inputs
        x, lambdas, group_e_list = inputs
        assert (
            len(group_e_list) == 2 * num_blocks + 1
        ...
        for i in range(1, num_blocks):
            ...
            block_e_list = group_e_list[
                1 + 2 * i : 1 + 2 * (i + 1)
            ]  # Next blocks have 2 layers
            x = basic_block(
                [x, lambdas, block_e_list],
                filters=filters,
                strides=1,
    ...
    # input conv layer
    e = e_models[0]
    x = Conv2D(16, strides=1, name="input_conv")(
        [input_data, input_lambdas, e(log_lambdas)]
    )
    ...
    x = Dense(num_classes, config.key_to_index, name="dense", activation=None)(
        [x, input_lambdas, e]
    )
