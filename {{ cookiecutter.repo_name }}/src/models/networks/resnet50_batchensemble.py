...
def bottleneck_block(
    ...
    if use_ensemble_bn:
        x = EnsembleBatchNormalization(
            ensemble_size=ensemble_size, name=bn_name_base + "2a"
        )(x)
    else:
    ...
    if use_ensemble_bn:
        x = EnsembleBatchNormalization(
            ensemble_size=ensemble_size, name=bn_name_base + "2b"
        )(x)
    else:
    ...
    if use_ensemble_bn:
        x = EnsembleBatchNormalization(
            ensemble_size=ensemble_size, name=bn_name_base + "2c"
        )(x)
    else:
    ...
    if not x.shape.is_compatible_with(shortcut.shape):
        ...
        if use_ensemble_bn:
            shortcut = EnsembleBatchNormalization(
                ensemble_size=ensemble_size, name=bn_name_base + "1"
            )(shortcut)
        else:
...
def resnet50_batchensemble(
    input_shape, num_classes, ensemble_size, random_sign_init, use_ensemble_bn
):
    ...
    if use_ensemble_bn:
        x = EnsembleBatchNormalization(ensemble_size=ensemble_size, name="bn_conv1")(x)
    else:
    ...
    return tf.keras.Model(inputs=inputs, outputs=x, name="resnet50_be")


def resnet101_batchensemble(
    input_shape, num_classes, ensemble_size, random_sign_init, use_ensemble_bn
):
    ...
    return tf.keras.Model(inputs=inputs, outputs=x, name="resnet101_be")


def resnet_batchensemble(
    input_shape,
    num_classes,
    ensemble_size,
    random_sign_init,
