...
def make_conv2d_layer(
    use_spec_norm: bool = True, spec_norm_iteration: int = 1, spec_norm_bound: float = 6
):
...
def basic_block(
    ...
    Args:
      ...
      filterwise_dropout: Whether to apply filterwise dropout.
    ...
    y = Conv2D(filters, strides=1, kernel_regularizer=tf.keras.regularizers.l2(l2))(y)
...
def wide_resnet_sngp(
    ...
    if use_mc_dropout and not filterwise_dropout:
    ...
    x = Conv2D(16, strides=1, kernel_regularizer=tf.keras.regularizers.l2(l2))(inputs)
    if filterwise_dropout:
    ...
    if use_gp_layer:
        ...
        outputs = GaussianProcess(num_classes)(x)[0]
    else:
