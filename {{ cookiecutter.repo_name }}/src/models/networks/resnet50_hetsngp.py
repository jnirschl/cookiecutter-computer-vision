...
def MonteCarloDropout(  # pylint:disable=invalid-name
    inputs, dropout_rate, use_mc_dropout, filterwise_dropout
):
...
def bottleneck_block(inputs, filters, stage, block, strides, conv_layer, dropout_layer):
...
def group(inputs, filters, num_blocks, stage, strides, conv_layer, dropout_layer):
...
def resnet50_hetsngp_add_last_layer(
    ...
    return tf.keras.Model(inputs=inputs, outputs=outputs, name="resnet50_hetsngp")


def resnet50_hetsngp(
    input_shape,
    batch_size,
    num_classes,
    num_factors,
