...
class WideResnetPosteriorNetworkTest(tf.test.TestCase):
    def testWideResnetPosteriorNetwork(self):
        ...
        class_counts = tf.cast(
            tf.math.bincount(tf.cast(tf.reshape(labels, -1), tf.int32)), tf.float32
        )

        for flow_type in ["maf", "affine", "radial"]:
            model = networks.wide_resnet_posterior_network(
            ...
            uce_loss_fn = ed.losses.uce_loss(sparse=True, num_classes=num_classes)
